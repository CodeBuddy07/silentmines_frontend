...
const AllProducts = () => {
    ...
    const handleUpdateProduct = async (e: any) => {
        ...
        try {
            setLoading(true);
            const response = await axios.put(`${baseUrl}/products/${updatedProduct._id}`, formData);
    ...
    const fetchProducts = async () => {
        try {
            const response = await axios.get(`http://localhost:5001/api/products?page=${currentPage}&limit=6`);
